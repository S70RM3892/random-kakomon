...
| F9 | 結果共有 | 終了後に全員の点数を並べて表示 | 実装済み |
| F10 | 履歴・分析 | 科目別の平均得点率、失点原因の割合、京大過去問の消化率 | 実装済み |
| F11 | 復習キュー | 失点した大問を「当日」「1週間後」に再出題リストへ自動追加 | 見送り |
| F12 | 演出 | ルーレットのアニメーション・効果音 | 実装済み |

受験目的で外せない仕様は F4 の京大寄せ重み付け。ルーレットの楽しさで量をこなしても、京大本番形式に偏らせなければ点数に直結しない。

F11 の復習キューは見送った。間違えた問題に戻る仕組みは持たず、F10 の履歴・分析（失点原因の割合と科目別の得点率）を見て自分で戻す運用にする。
...
1. 対象は約12大学に絞る（京大、東大、阪大、東京科学大、東北大、名大、九大、北大、神戸、一橋、早稲田、慶應）
   - 一橋は文系数学だが、整数・確率の良問が理系の演習にも使えるので入れる
...
4. デプロイは GitHub Pages。main への push で GitHub Actions が静的書き出しをビルドして公開する（Vercel アカウント不要。公開URLは <https://s70rm3892.github.io/random-kakomon/>）

GitHub 連携でも片付かない、自分でやる作業（合計30分程度）:

- [ ] Firebase または Supabase のプロジェクト作成とキー発行
- [ ] キーは GitHub の Actions Variables に入れる。リポジトリのファイルには絶対に書かない（コードは `.env` 参照にさせる）
...
- [x] MVP: F1〜F8（ルーム、抽選、京大寄せ重み、重複回避、タイマー、リンク、結果入力）
- [x] 2版目: F9（結果共有）、F10（履歴・分析）、F12（演出）
- [ ] 見送り: F11（復習キュー）。テーブル review_items だけ先に置いてある
